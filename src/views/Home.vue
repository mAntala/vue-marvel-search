    ...
    <section class="page home">
        <div class="container">
            ...
            <span class="hidden-text">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 654.48 249.84">
            ...
            <!-- Search input -->
            <search-input v-model="searchInput" @update-search="updateSearchInput" />

            <!-- Search suggestions -->
            <search-suggestions @update-search="updateSearchInput" @suggestion-click="suggestionClickEvent" />
        </div>
    </section>
</template>
...
export default {
    ...
    methods: {
        // Update searchInput data from child component
        updateSearchInput(newValue) {
            this.searchInput = newValue;
            if(newValue !== '') {
                this.$store.commit('setSearchedItem', newValue);
            }
        },
        // When user click on suggestion, he doesn't have to click or hit enter (for better UX i think)
        // He will be redirected to search results for that hero
        suggestionClickEvent(suggestion) {
            this.$router.push({name: 'Search', query: {hero: suggestion}});
        }
    }
}
</script>
